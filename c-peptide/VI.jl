# Model fit to the train data and evaluation on the test data using ADVI
train_model = true
figures = true
folder = "VI"  # Changed from "MLE" to "VI" for Variational Inference
dataset = "ohashi_low"
extension = "png"
pt = 4 / 3
...
include("src/c_peptide_ude_models_VI.jl")  # Use the ADVI version
...
if train_model
    advi_results_train = train(models_train[indices_train], train_data.timepoints, train_data.cpeptide[indices_train, :], rng, initial_guesses=25_000, selected_initials=3)

    # Extract neural network parameters and betas from ADVI results
    neural_network_parameters = []
    betas = []

    for result in advi_results_train
        # For ADVI results, we need to extract the neural network parameters and betas
        # The mean_params contains both neural and ode parameters
        n_neural = length(models_train[1].chain |> p -> init_params(p, rng=rng))
        neural_params = result.mean_params[1:n_neural]
        beta_params = result.mean_params[(n_neural+1):end]

        push!(neural_network_parameters, neural_params)
        push!(betas, beta_params)
    end
    ...
    best_advi_result = advi_results_train[best_model_index]

    # Extract the best neural network parameters
    n_neural = length(models_train[1].chain |> p -> init_params(p, rng=rng))
    neural_network_parameters = best_advi_result.mean_params[1:n_neural]
    ...
    jldopen("data/$folder/cude_neural_parameters$dataset.jld2", "w") do file
        ...
        file["advi_result"] = best_advi_result  # Save the full ADVI result
    end
else
    neural_network_parameters, betas, best_model_index, best_advi_result = try
        jldopen("data/$folder/cude_neural_parameters$dataset.jld2") do file
            file["parameters"], file["betas"], file["best_model_index"], file["advi_result"]
        end
    catch
...
advi_results_train = train(models_train, train_data.timepoints, train_data.cpeptide, neural_network_parameters,
    advi_lower_bound=lb, advi_upper_bound=ub)
betas_train = [result.mean_params[1] for result in advi_results_train]
objectives_train = [result.objective for result in advi_results_train]
...
advi_results_test = train(models_test, test_data.timepoints, test_data.cpeptide, neural_network_parameters,
    advi_lower_bound=lb, advi_upper_bound=ub)
betas_test = [result.mean_params[1] for result in advi_results_test]
objectives_test = [result.objective for result in advi_results_test]
...
# save betas 
jldopen("data/VI/betas_$dataset.jld2", "w") do file
    file["train"] = betas_train
    file["test"] = betas_test
    file["train_std"] = [result.std_params[1] for result in advi_results_train]  # Save uncertainty
    file["test_std"] = [result.std_params[1] for result in advi_results_test]   # Save uncertainty
end

# save mse
jldopen("data/VI/mse_$dataset.jld2", "w") do file
...
# Save R² values
jldopen("data/VI/r2_$dataset.jld2", "w") do file
    file["train"] = r2_train
    file["test"] = r2_test
end
...
println("Mean β uncertainty (train): $(round(mean([result.std_params[1] for result in advi_results_train]), digits=4))")
println("Mean β uncertainty (test): $(round(mean([result.std_params[1] for result in advi_results_test]), digits=4))")
...
if figures
    ...
    individual_fits_figure = let fig
        ...
        fig = Figure(size=(200 * n_cols, 150 * n_rows),
            title="Individual Model Fits (ADVI)",
            fontsize=8pt,
            font=FONTS.regular)
    ...
    # ADVI-specific: Create uncertainty quantification plot
    uncertainty_figure = let fig
        fig = Figure(size=(800, 600), fontsize=8pt, font=FONTS.regular)

        # Beta uncertainty plot
        ax_beta_unc = Axis(fig[1, 1],
            xlabel="β mean",
            ylabel="β standard deviation",
            title="β Parameter Uncertainty (ADVI)")

        # Training data uncertainty
        train_means = betas_train
        train_stds = [result.std_params[1] for result in advi_results_train]

        # Test data uncertainty  
        test_means = betas_test
        test_stds = [result.std_params[1] for result in advi_results_test]

        # Scatter plot colored by data type
        for (i, type) in enumerate(unique(train_data.types))
            type_indices = train_data.types .== type
            scatter!(ax_beta_unc, train_means[type_indices], train_stds[type_indices],
                color=(Makie.wong_colors()[i], 0.7), markersize=8,
                label="Train $(type)", marker=:circle)
        end

        for (i, type) in enumerate(unique(test_data.types))
            type_indices = test_data.types .== type
            scatter!(ax_beta_unc, test_means[type_indices], test_stds[type_indices],
                color=(Makie.wong_colors()[i], 0.4), markersize=6,
                label="Test $(type)", marker=:utriangle)
        end

        axislegend(ax_beta_unc, position=:rt)

        # ELBO convergence plot
        ax_elbo = Axis(fig[2, 1],
            xlabel="Iteration",
            ylabel="ELBO",
            title="ELBO Convergence (Sample Subject)")

        # Show ELBO trace for a representative subject
        if length(advi_results_test) > 0 && length(advi_results_test[1].elbo_trace) > 0
            representative_idx = argmedian(objectives_test)
            lines!(ax_elbo, 1:length(advi_results_test[representative_idx].elbo_trace),
                advi_results_test[representative_idx].elbo_trace,
    ...
    save("figures/$folder/uncertainty_analysis_$dataset.$extension", uncertainty_figure, px_per_unit=4)
